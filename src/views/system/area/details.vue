<template>
  <ACard :body-style="{ padding: '16px' }" class="dept-card" title="详情">
    ...
    <a-descriptions bordered :column="2" v-if="info.id">
      <a-descriptions-item label="父区划编号">{{ info.pid }}</a-descriptions-item>
      <a-descriptions-item label="父区划名称">{{ info.pname }}</a-descriptions-item>
      <a-descriptions-item label="区划编号">{{ info.id }}</a-descriptions-item>
      <a-descriptions-item label="区划名称" v-if="!editMode">
        {{ info.name }}
        <a-button :icon="h(EditOutlined)" @click="editMode = true" class="ml-2" />
      </a-descriptions-item>
      <a-descriptions-item label="区划名称" v-else>
...
import { getAreaInfo, deleteAreaInfo, modifyAreaInfo } from '@/api/system/area'
import AddForm from '@/views/system/area/add-form.vue'
...
watch(
  ...
  async (newValue, oldValue) => {
    if (newValue) {
      ...
      info.value = await getAreaInfo(newValue)
...
const modifyMethod = async () => {
  await modifyAreaInfo(info.value.id, newName.value)
  info.value = await getAreaInfo(info.value.id)
  newName.value = info.value.name
  editMode.value = false
}
</script>
