  ...
  >
    ...
    >
      <ASpin :spinning="loading">
        <ARow :gutter="16">
          <ACol :lg="12" :span="24">
            <AFormItem label="父区划编号" name="pid">
              <AInput v-model:value="formData.pid" disabled />
            </AFormItem>
          </ACol>
          <ACol :lg="12" :span="24">
            <AFormItem label="父区划名称" name="pname">
              <AInput v-model:value="formData.pname" disabled />
            </AFormItem>
          </ACol>
          <ACol :lg="12" :span="24">
            <AFormItem label="区划名称" name="name">
              <AInput v-model:value="formData.name" />
            </AFormItem>
          </ACol>
          <ACol :lg="12" :span="24">
            <AFormItem label="区划编码" name="code">
...
import { addAreaInfo } from '@/api/system/area'
const refresh = inject('layoutContext')

const formRef = ref<FormInstance>()

const rules: FormProps['rules'] = {
  name: [{ required: true, message: '请填写区划名称' }]
}
...
const submit = async () => {
  try {
    ...
    const data = await addAreaInfo(formData.value)
    console.log(data)
