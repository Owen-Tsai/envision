<template>
  <ACard :body-style="{ padding: '16px' }" class="dept-card" title="详情">
    ...
    <a-descriptions bordered :column="2" v-if="info.id">
      <a-descriptions-item label="父专业编号">{{ info.pid }}</a-descriptions-item>
      <a-descriptions-item label="父专业名称">{{ info.pname }}</a-descriptions-item>
      <a-descriptions-item label="专业编号">{{ info.id }}</a-descriptions-item>
      <a-descriptions-item label="专业名称" v-if="!editMode">
        {{ info.name }}
        <a-button :icon="h(EditOutlined)" @click="editMode = true" class="ml-2" />
      </a-descriptions-item>
      <a-descriptions-item label="专业名称" v-else>
...
import { getMajorInfo, deleteMajorInfo, modifyMajorInfo } from '@/api/system/major'
import AddForm from '@/views/system/major/add-form.vue'
...
watch(
  ...
  async (newValue, oldValue) => {
    if (newValue) {
      ...
      info.value = await getMajorInfo(newValue)
...
const modifyMethod = async () => {
  await modifyMajorInfo(info.value.id, newName.value)
  info.value = await getMajorInfo(info.value.id)
  newName.value = info.value.name
  editMode.value = false
}
</script>
