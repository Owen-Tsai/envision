  ...
  >
    ...
    >
      <ASpin :spinning="loading">
        <ARow :gutter="16">
          <ACol :lg="12" :span="24">
            <AFormItem label="父专业编号" name="pid">
              <AInput v-model:value="formData.pid" disabled />
            </AFormItem>
          </ACol>
          <ACol :lg="12" :span="24">
            <AFormItem label="父专业名称" name="pname">
          ...
          <ACol :lg="12" :span="24">
            <AFormItem label="专业编码" name="code">
...
import { addMajorInfo } from '@/api/system/major'
...
const submit = async () => {
  try {
    ...
    const data = await addMajorInfo(formData.value)
    console.log(data)
