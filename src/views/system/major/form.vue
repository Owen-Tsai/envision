    ...
    <div>
      <ASpin v-if="id" :spinning="loading">
        ...
        >
          <ARow>
            <ACol :span="24" :lg="12">
              <AFormItem label="上级专业编号" name="pid">
                {{ formData.pid }}
              </AFormItem>
            </ACol>
            <ACol :span="24" :lg="12">
              <AFormItem label="上级专业名称" name="pname">
                {{ formData.pname }}
              </AFormItem>
            </ACol>
          </ARow>
          <ARow>
            <ACol :span="24" :lg="12">
              <AFormItem label="专业代码" name="code">
                <AInput v-model:value="formData.code" />
              </AFormItem>
            </ACol>
            <ACol :span="24" :lg="12">
              <AFormItem label="专业名称" name="name">
      ...
      <AEmpty v-else description="选择一个专业进行编辑" />
    </div>
  </ACard>
</template>
...
const title = computed(() =>
  mode.value === 'add' ? `在${formData.value.pname}下新增专业` : '编辑专业',
)

const formData = ref<MajorTreeVO>({})
...
const submit = async () => {
  ...
  try {
    ...
    if (mode.value === 'add') {
      const id = await addMajorInfo(formData.value)
      emit('update:id', id)
      mode.value = 'edit'
    } else {
      await modifyMajorInfo(formData.value)
    }
    emit('success')
